...
class CSStepper : UIControl {
    ...
    public var value: Int = 0 {
        didSet {
            ...
            self.sendActions(for: .valueChanged)    // 이 클래스를 사용하는 객체들에게 valueChange 이벤트 신호를 준다. UIView 대신 UIControl을 쓰는 이유
        }
    }
    ...
    override public func layoutSubviews() {
        ...
        let btnWidth = self.frame.height    // 버튼의 너비 = 뷰 넓이
        let lblWidth = self.frame.width - (btnWidth * 2)    // 레이블너비 = 뷰크기 - 버튼양쪽 너비합
